...
export class UserRepository extends Repository {
    ...
    create({ CPF, email, password, username, id }: UserInput & { id: string }) {
        ...
        const insert = this.db.prepare("INSERT INTO users (username, email, password, CPF, id) VALUES (?1, ?2, ?3, ?4, ?5);")
    ...
    getUserInfos(id: string) {
        const user = this.db.query(`SELECT * FROM users WHERE id = ?1;`).get(id)
